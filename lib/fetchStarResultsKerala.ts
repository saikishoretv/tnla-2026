import { fetchKeralaConstituencyDetail } from "./eci";
import { STAR_CANDIDATES_KERALA, type StarCandidateKerala } from "./starCandidatesKerala";
import type { CandidateResult } from "./types";

export interface StarCandidateKeralaResult {
  meta: StarCandidateKerala;
...
export async function fetchAllKeralaStarResults(): Promise<StarCandidateKeralaResult[]> {
  const details = await Promise.all(
    STAR_CANDIDATES_KERALA.map((sc) => fetchKeralaConstituencyDetail(sc.acId))
  );

  return STAR_CANDIDATES_KERALA.map((meta, i) => {
