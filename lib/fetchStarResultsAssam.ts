import { fetchAssamConstituencyDetail } from "./eci";
import { STAR_CANDIDATES_ASSAM, type StarCandidateAssam } from "./starCandidatesAssam";
import type { CandidateResult } from "./types";

export interface StarCandidateAssamResult {
  meta: StarCandidateAssam;
...
export async function fetchAllAssamStarResults(): Promise<StarCandidateAssamResult[]> {
  const details = await Promise.all(
    STAR_CANDIDATES_ASSAM.map((sc) => fetchAssamConstituencyDetail(sc.acId))
  );

  return STAR_CANDIDATES_ASSAM.map((meta, i) => {
