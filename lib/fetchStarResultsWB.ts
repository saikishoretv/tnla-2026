import { fetchWBConstituencyDetail } from "./eci";
import { STAR_CANDIDATES_WB, type StarCandidateWB } from "./starCandidatesWB";
import type { CandidateResult } from "./types";

export interface StarCandidateWBResult {
  meta: StarCandidateWB;
...
export async function fetchAllWBStarResults(): Promise<StarCandidateWBResult[]> {
  const details = await Promise.all(
    STAR_CANDIDATES_WB.map((sc) => fetchWBConstituencyDetail(sc.acId))
  );

  return STAR_CANDIDATES_WB.map((meta, i) => {
