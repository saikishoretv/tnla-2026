/**
 * Fetches Kerala 2026 election data and writes JSON files to data/kerala/
 * Run from your laptop: node scripts/fetch-kerala.mjs
 */
...
const PARTY_FULL_NAMES = {
  ...
  RMP: "Revolutionary Marxist Party of India",
...
function getDistrict(acNo) {
  const map = [
    [1, 7, "Thiruvananthapuram"],
    [8, 16, "Kollam"],
    [17, 23, "Pathanamthitta"],
    [24, 33, "Alappuzha"],
    [34, 43, "Kottayam"],
    [44, 52, "Idukki"],
    [53, 63, "Ernakulam"],
    ...
    [112, 121, "Kannur"],
    [122, 130, "Kasaragod"],
    [131, 140, "Thiruvananthapuram"], // fallback
  ];
...
// Main
console.log("Fetching Kerala 2026 data...");
...
mkdirSync("data/kerala", { recursive: true });
mkdirSync("data/kerala/constituency", { recursive: true });
writeFileSync("data/kerala/summary.json", JSON.stringify(summary));
writeFileSync("data/kerala/constituencies.json", JSON.stringify(constituencies));

console.log("Fetching all 140 constituency details...");
const details = await fetchAllConstituencyDetails();
for (const d of details) {
  writeFileSync(`data/kerala/constituency/${d.id}.json`, JSON.stringify(d));
}
