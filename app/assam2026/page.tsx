...
async function DashboardContent() {
  ...
  const allianceBadges = [
    ...
    { label: "INDIA Alliance", seats: allianceTotals["INDIA Alliance"] ?? 0, color: "#3B82F6" },
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">Assam Assembly Election 2026</h1>
          <p className="text-gray-400 text-sm mt-1">
            {summary.statusLine ? `Status known for ${summary.statusLine}` : "126 Assembly Constituencies"}
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <Link href="/assam2026/results"
          ...
          View all 126 constituency results <span aria-hidden>→</span>
        </Link>
        <Link href="/assam2026/star-candidates"
          className="inline-flex items-center gap-2 px-6 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl text-sm font-medium transition-colors border border-gray-700">
...
export default function AssamHomePage() {
  return (
