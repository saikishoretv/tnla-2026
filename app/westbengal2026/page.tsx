...
async function DashboardContent() {
  ...
  const allianceBadges = [
    { label: "NDA", seats: allianceTotals["NDA"] ?? 0, color: "#F97316" },
    { label: "TMC Alliance", seats: allianceTotals["TMC Alliance"] ?? 0, color: "#10B981" },
    { label: "Left+Congress", seats: allianceTotals["Left+Congress"] ?? 0, color: "#3B82F6" },
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">West Bengal Assembly Election 2026</h1>
          <p className="text-gray-400 text-sm mt-1">
            {summary.statusLine ? `Status known for ${summary.statusLine}` : "294 Assembly Constituencies"}
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <Link href="/westbengal2026/results"
          ...
          View all 294 constituency results <span aria-hidden>→</span>
        </Link>
        <Link href="/westbengal2026/star-candidates"
          className="inline-flex items-center gap-2 px-6 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl text-sm font-medium transition-colors border border-gray-700">
          Star candidates & ministers <span aria-hidden>→</span>
        </Link>
      </div>
    </>
  );
...
export default function WestBengalHomePage() {
  return (
